...
void BFrustum::makeVolume(BOCamera * camera)
{
	// get viewport
	...
	// front normal
	m_direction = camera->getRotatedVector(BVector3f(0, 0, -1)).getNormalized();

 	// compute volume points
	BVector3f position = camera->getTransformedPosition();
	...
	else
	{
		...
		m_normals[0] = camera->getRotatedVector(BVector3f(1, 0, 0)).getNormalized();
		m_normals[2] = - m_normals[0];
		m_normals[1] = camera->getRotatedVector(BVector3f(0, 1, 0)).getNormalized();
		m_normals[3] = - m_normals[1];
	}
}

bool BFrustum::isVolumePointsVisible(BVector3f * points, unsigned int pointsNumber)
{
	bool out = true;
	unsigned int p;
	...
	for(int i=0; i<4; i++)
	{
		BVector3f * origin = &m_points[pointsId[i][1]];
