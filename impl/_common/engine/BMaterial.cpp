...
BMaterial::BMaterial(const BMaterial & material):
...
void BMaterial::allocTexturesPass(unsigned int size)
{
	clearTexturesPass();
	if(size == 0)
		return;
	m_texturesPass = (BTexturePass **) malloc(sizeof(void *)*size);
}

void BMaterial::clearTexturesPass(void)
{
	unsigned int i;
...
void BMaterial::addTexturePass(BTexture * texture, M_TEX_COMBINE_MODES combineMode, unsigned int mapChannel)
{
	m_texturesPass[m_texturesPassNumber] = new BTexturePass(texture, combineMode, mapChannel);
