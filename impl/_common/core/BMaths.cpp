...
bool isEdgeToEdge2dIntersection(const BVector2f & A, const BVector2f & B, const BVector2f & C, const BVector2f & D, BVector2f * I)
{
	/*
	BVector2f min1(MIN(A.x, B.x), MIN(A.y, B.y));
	BVector2f max1(MAX(A.x, B.x), MAX(A.y, B.y));
	BVector2f min2(MIN(C.x, D.x), MIN(C.y, D.y));
	BVector2f max2(MAX(C.x, D.x), MAX(C.y, D.y));
...
bool isLineCircleIntersection(const BVector2f & origin, const BVector2f & dest, const BVector2f & circleCenter, float circleRadius)
{
	...
	BVector2f vec = dest - origin;
	BVector2f N = BVector2f(-vec.y, vec.x).getNormalized();
...
bool isBoxToBoxCollision(const BVector3f & minA, const BVector3f & maxA, const BVector3f & minB, const BVector3f & maxB)
{
	// X
...
bool isBoxToBox2dCollision(const BVector2f & minA, const BVector2f & maxA, const BVector2f & minB, const BVector2f & maxB)
{
	// X
	if(minA.x > maxB.x) return false;
	if(maxA.x < minB.x) return false;
...
bool isPointInBox(const BVector3f & point, const BVector3f & min, const BVector3f & max)
{
...
bool isPointInBox2d(const BVector2f & point, const BVector2f & min, const BVector2f & max)
{
	if(point.x > max.x)
		return false;
...
bool isEdgeToBoxCollision(const BVector3f & origin, const BVector3f & dest, const BVector3f & min, const BVector3f & max)
{
	BVector3f nrm[6];
	BVector3f pts[6];
	BVector3f pt;
...
bool isEdgePlaneIntersection(const BVector3f & origin, const BVector3f & dest, const BVector3f & planePoint, const BVector3f & normal, BVector3f * point)
{
	BVector3f vec_tmp[2], vec[2];
...
bool isRaySphereIntersection(const BVector3f & origin, const BVector3f & direction, const BVector3f & sphereCenter, float sphereRadius, BVector3f * point)
{
	BVector3f vec = origin - sphereCenter;
...
bool isRayPlaneIntersection(const BVector3f & origin, const BVector3f & direction, const BVector3f & planePoint, const BVector3f & planeNormal, BVector3f * point)
{
...
bool isPointInTriangle(const BVector3f & point, const BVector3f & a, const BVector3f & b, const BVector3f & c, const BVector3f & normal)
{
	BVector3f nrm = getTriangleNormal(point, a, b);
...
bool isEdgeTriangleIntersection(const BVector3f & origin, const BVector3f & dest, const BVector3f & a, const BVector3f & b, const BVector3f & c, const BVector3f & normal, BVector3f * point)
{
	BVector3f iPoint;
...
BVector3f getTriangleNormal(const BVector3f & a, const BVector3f & b, const BVector3f & c)
{
	BVector3f vec0 = a - b;
	BVector3f vec1 = a - c;
...
BVector3f RGBToHSV(BVector3f rgbColor)
{
	float r = rgbColor.x;
	float g = rgbColor.y;
	float b = rgbColor.z;
...
BVector3f HSVToRGB(BVector3f HSVColor)
{
	int i;
	float r, g, b;
	float f, p, q, t;
...
BVector3f RGBToHSL(BVector3f rgbColor)
{
...
BVector3f HSLToRGB(BVector3f hslColor)
{
	float H = hslColor.x;
	float S = hslColor.y;
	float L = hslColor.z;
...
// Modified by Anaël Seghezzi to use BVector3f
...
void simplifyDP(float tol, BVector3f * v, int j, int k, int * mk)
{
    ...
    BVector3f	S[2] = {v[j], v[k]};		// segment from v[j] to v[k]
    BVector3f	u = S[1] - S[0];			// segment direction vector
    ...
    BVector3f w;
    BVector3f Pb;               // base of perpendicular from v[i] to S
