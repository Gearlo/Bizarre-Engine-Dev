...
void BQuaternion::normalize(void)
{
...
void BQuaternion::loadIdentity(void)
{
	values[0] = 0.0f;
	values[1] = 0.0f;
	values[2] = 0.0f;
...
BQuaternion BQuaternion::operator * (const BQuaternion & quat) const
{
	return BQuaternion(
...
void BQuaternion::operator *= (const BQuaternion & quat)
{
	(*this) = (*this) * quat;
}

void BQuaternion::invert()
{
	values[0] = -values[0];
	values[1] = -values[1];
	values[2] = -values[2];
...
void BQuaternion::setFromAngles(float xAngle, float yAngle, float zAngle)
{
...
void BQuaternion::slerp(const BQuaternion & q1, const BQuaternion & q2, float interpolation)
{
	BQuaternion q3;
...
void BQuaternion::setFromAngleAxis(float angle, const BVector3f & axis)
{
	const BVector3f normAxis = axis.getNormalized();
...
void BQuaternion::setFromVectors(const BVector3f & source, const BVector3f & destination)
{
	BVector3f axis=source.crossProduct(destination);
...
float BQuaternion::getAngle(void) const
{
	float v = values[3];
...
BVector3f BQuaternion::getAxis(void) const
{
	float v = values[3];
	...
	return BVector3f(0.0f, 1.0f, 0.0f);
}

BVector3f BQuaternion::getEulerAngles(void) const
{
	BMatrix4x4 matrix;
