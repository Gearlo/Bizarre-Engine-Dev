...
static BFileOpenHook * s_fileOpenHook = 0;



bool copyFile(const char * inFilename, const char * outFilename)
{
	BFile * in = M_fopen(inFilename, "rb");
	if(! in)
	{
	...
	BFile * out = M_fopen(outFilename, "wb");
	if(! out)
	{
...
void M_registerFileOpenHook(BFileOpenHook * hook)
{		
	s_fileOpenHook = hook;
}

BFileOpenHook* M_getFileOpenHook()
{
	return s_fileOpenHook;
}

BFile * M_fopen(const char * path, const char * mode)
{
	BFile * rtn;
	if(s_fileOpenHook)
		rtn = s_fileOpenHook->open(path, mode);
	else
		rtn = BStdFile::getNew(path, mode);
...
size_t M_fread(void * dest, size_t size, size_t count, BFile * stream)
{
	if(stream)
...
size_t M_fwrite(const void * str, size_t size, size_t count, BFile * stream)
{
	if(stream)
...
int M_fprintf(BFile * stream, const char * format, ...)
{
	va_list args;
	if(stream)
	{
...
int M_fseek(BFile * stream, long offset, int whence)
{
	if(stream)
