...
BJoystickLinux::BJoystickLinux(const char* joydev)
{
	if(joydev == NULL)
	{
		return;
...
BJoystickLinux::~BJoystickLinux()
{
	
}

int BJoystickLinux::getAxis(int idx)
{
	if(idx < m_numAxes)
	{
		return m_axis[idx];
...
float BJoystickLinux::getDeltaAxis(int idx)
{
	if(idx < m_numAxes)
	{
		return m_dAxis[idx];
...
bool BJoystickLinux::getButton(int idx)
{
	if(idx < m_numButtons)
	{
...
void BJoystickLinux::updateData()
{
	if(m_file == -1)
		return;
