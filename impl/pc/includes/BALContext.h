...
class BALContext : public BSoundContext
{
private:
...
public:
	...
	void sendBufferSound(unsigned int bufferId, BSound * sound);
	void sendBufferData(unsigned int bufferId, B_SOUND_FORMAT format, void * data, unsigned int size, unsigned int freq);
	...
	void setSourcePosition(unsigned int sourceId, const BVector3f & position);
	...
	// listener
	void updateListenerPosition(const BVector3f & position, const BVector3f & direction, const BVector3f & up);
};
