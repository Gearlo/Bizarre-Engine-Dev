...
static void drawBitmap(BImage * image, FT_Bitmap * bitmap, int left, int top)
{
	int x, y;
...
bool B_loadFont(const char * filename, void * data)
{
	int pen_x, pen_y;
	unsigned int n;
	...
	for(n = 0; n<256; n++)
	{
		...
		BVector2f offset = BVector2f((float)slot->bitmap_left, - (float)slot->bitmap_top) / ((float)size);
		BVector2f pos = BVector2f((float)(pen_x-1) / (float)width, (float)(pen_y-1) / (float)height);
		BVector2f scale = BVector2f((float)(slot->bitmap.width+2) / (float)width, (float)(slot->bitmap.rows+2) / (float)height);

		// set character
		font->setCharacter(n, BCharacter(xAdvance, offset, pos, scale));
