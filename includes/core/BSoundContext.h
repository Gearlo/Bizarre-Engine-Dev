...
enum B_SOUND_FORMAT
{
	B_SOUND_FORMAT_MONO8 = 0,
	B_SOUND_FORMAT_MONO16,
	B_SOUND_FORMAT_STEREO8,
	B_SOUND_FORMAT_STEREO16
};


class M_CORE_EXPORT BSoundContext
{
public:

	// destructor
	virtual ~BSoundContext(void){}
	...
	virtual void sendBufferSound(unsigned int bufferId, BSound * sound) = 0;
	virtual void sendBufferData(unsigned int bufferId, B_SOUND_FORMAT format, void * data, unsigned int size, unsigned int freq) = 0;
	...
	virtual void setSourcePosition(unsigned int sourceId, const BVector3f & position) = 0;
	...
	// listener
	virtual void updateListenerPosition(const BVector3f & position, const BVector3f & direction, const BVector3f & up) = 0;
};
