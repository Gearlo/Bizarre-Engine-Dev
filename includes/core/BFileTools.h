...
// file wrapper
M_CORE_EXPORT void M_registerFileOpenHook(BFileOpenHook * hook);
M_CORE_EXPORT BFileOpenHook * M_getFileOpenHook();

M_CORE_EXPORT BFile* M_fopen(const char * path, const char* mode);
M_CORE_EXPORT int	 M_fclose(BFile * stream);
M_CORE_EXPORT size_t M_fread(void * dest, size_t size, size_t count, BFile * stream);
M_CORE_EXPORT size_t M_fwrite(const void * str, size_t size, size_t count, BFile * stream);
M_CORE_EXPORT int	 M_fprintf(BFile * stream, const char * format, ...);
M_CORE_EXPORT int	 M_fseek(BFile * stream, long offset, int whence);
M_CORE_EXPORT long	 M_ftell(BFile * stream);
M_CORE_EXPORT void	 M_rewind(BFile * stream);
