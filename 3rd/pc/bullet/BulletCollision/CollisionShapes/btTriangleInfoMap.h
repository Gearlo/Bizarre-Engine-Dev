...
SIMD_FORCE_INLINE	const char*	btTriangleInfoMap::serialize(void* dataBuffer, btSerializer* serializer) const
{
	...
	tmapData->m_nuBKeys = m_keyArray.size();
	tmapData->m_keyArrayPtr = tmapData->m_nuBKeys ? (int*)serializer->getUniquePointer((void*)&m_keyArray[0]) : 0;
...
SIMD_FORCE_INLINE	void	btTriangleInfoMap::deSerialize(btTriangleInfoMapData& tmapData )
{
	...
	m_keyArray.resize(tmapData.m_nuBKeys,btHashInt(0));
	for (i=0;i<tmapData.m_nuBKeys;i++)
	{
